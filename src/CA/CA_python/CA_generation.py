...
# sbert = SentenceTransformer("bert-all-MiniLM-L6-v2")
sbert = SentenceTransformer('bert-all-MiniLM-L6-v2')
...
def generate_topics(
  ...
  #  topic_format = regex.compile(r"^$(\d+)$\s*([\w\s\-]+):\s*(.+)$")
...
def generate_topic_lvl1(
    api, model, data, prompt_file, seed_file, out_file, topic_file, verbose,key
):
    """
    ...
    """
    api_client = APIClient(api=api, model=model,key=key)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--api",
        type=str,
        help="API to use ('openai', 'vertex', 'vllm', 'azure', 'gemini','deepseek-chat')",
        default="deepseek-chat",
    )
    parser.add_argument("--model", type=str, help="Model to use", default="openai")

    parser.add_argument(
        "--data",
        type=str,
        default=f"../data/input/climate_{stance}_1000_16.jsonl",
        help="Data to run generation on",
    )
    parser.add_argument(
        "--prompt_file",
        type=str,
        default=f"../prompt/generation_{stance}.txt",
        help="File to read prompts from",
    )
    parser.add_argument(
        "--seed_file",
        type=str,
        default=f"../prompt/seed_{stance}.md",
        help="Markdown file to read seed topics from",
    )
    parser.add_argument(
        "--out_file",
        type=str,
        default=f"../data/output/generation_{stance}.jsonl",
        help="File to write results to",
    )
    parser.add_argument(
        "--topic_file",
        type=str,
        default=f"../data/output/generation_{stance}.md",
