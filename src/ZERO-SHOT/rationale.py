...
def get_ans(author_parent, author_child, submission, subreddit, pb, cb, label):
    ...
    instruction_carp = 'You are a helpful assistant at (Dis)agreement Detection, given parent_comment and child_reply from the post in the subreddit.\
    ...
    while attempt < max_retries:
        try:
            ...
            if attempt == max_retries:
                raise e
            delay = min(base_delay * 2 ** attempt + uniform(0, 0.2), 60)  # 计算延迟时间，并添加抖动避免同步
